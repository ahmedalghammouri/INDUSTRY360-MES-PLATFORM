...
function AutoGenerateDialog({ po, onClose, onDone }: { po: PipelinePO; onClose: () => void; onDone: () => void }) {
  const [start, setStart] = useState(toLocalInput(po.plannedStart));
  const [end, setEnd] = useState(toLocalInput(po.plannedEnd ?? new Date(Date.now() + 86_400_000).toISOString()));
  ...
  const preview: any = (previewData as any) ?? null;
  ...
  const steps: any[] = preview?.jobOrdersToCreate ?? [];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      >
        <div className="p-4 border-b flex items-center justify-between">
          <div>
            <h2 className="font-semibold text-sm flex items-center gap-1.5"><Zap size={14} className="text-primary" />Auto-Generate Work Order</h2>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              {po.orderNumber} · {po.sku?.name ?? '—'} · {po.targetQty} {po.unit ?? ''}
            </p>
          </div>
        ...
        <div className="p-4 flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1">
              <span className="text-[11px] font-medium">Production Start *</span>
              <Input type="datetime-local" value={start} onChange={e => setStart(e.target.value)} className="h-8 text-xs" />
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[11px] font-medium">Production End *</span>
              <Input type="datetime-local" value={end} onChange={e => setEnd(e.target.value)} className="h-8 text-xs" />
            </div>
          </div>

          {isLoading ? (
            <div className="text-xs text-muted-foreground py-6 text-center">Resolving process & routing…</div>
          ) : (
            <>
              {preview?.process && (
                <div className="flex items-center gap-2 text-xs p-2 rounded-lg bg-emerald-500/5 border border-emerald-500/25">
                  <Workflow size={13} className="text-emerald-400 shrink-0" />
                  <span>
                    Process: <strong>{preview.process.name}</strong> v{preview.process.version}
                    {preview.process.scopeType && preview.process.scopeType !== 'PRODUCT' && (
                      <span className="ml-1.5 text-[10px] text-emerald-400">({preview.process.scopeType.replace('_', ' ')} scope)</span>
                    )}
                  </span>
                </div>
              )}
              {preview?.warning && (
                <div className="flex items-start gap-2 text-[11px] p-2 rounded-lg bg-amber-500/5 border border-amber-500/30 text-amber-300">
              ...
              <div className="rounded-lg border overflow-hidden">
                <div className="px-3 py-2 bg-muted/30 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                  Job Orders ({steps.length})
                </div>
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border/30 text-[10px] text-muted-foreground">
                      ...
                      <th className="text-left px-3 py-1.5 font-medium">Machine / Cell</th>
                      <th className="text-right px-3 py-1.5 font-medium">Qty Flow</th>
                      <th className="text-right px-3 py-1.5 font-medium">Est. Duration</th>
                    </tr>
                  </thead>
                  <tbody>
                    {steps.map((s: any, i: number) => (
                      <tr key={i} className="border-b border-border/10">
                        <td className="px-3 py-1.5 font-mono text-primary">{s.stepNumber}</td>
                        <td className="px-3 py-1.5 font-medium">{s.operationName}</td>
                        <td className="px-3 py-1.5 text-muted-foreground">{s.machine?.name ?? s.workCenter?.name ?? '—'}</td>
                        <td className="px-3 py-1.5 text-right tabular-nums">
                          {s.plannedQtyIn != null && s.inputUnit
                            ? <>{s.plannedQtyIn} {s.inputUnit} <ArrowRight size={9} className="inline mx-0.5 text-muted-foreground" /> {s.plannedQtyOut} {s.outputUnit}</>
                            : <>{s.plannedQtyOut ?? s.plannedQty ?? '—'} {s.outputUnit ?? ''}</>}
                        </td>
                        <td className="px-3 py-1.5 text-right tabular-nums text-muted-foreground">
                          {s.estimatedDurationMins != null ? `${Math.round(s.estimatedDurationMins)} min` : '—'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              {(preview?.existingWOCount ?? 0) > 0 && (
                <p className="text-[11px] text-amber-400">This PO already has {preview.existingWOCount} work order(s).</p>
              )}
            </>
          )}
        </div>
        ...
        <div className="p-4 border-t flex justify-end gap-2">
          ...
          <Button
            size="sm"
            disabled={generateMutation.isPending || !preview?.canGenerate}
